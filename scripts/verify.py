...
def main() -> None:
    ...
    if broken_parents:
        ...
        extra = len(broken_parents) - len(preview)
        more = f" (+{extra} more)" if len(broken_parents) > len(preview) else ""
        errors.append(f"broken parent refs ({len(broken_parents)}): {preview}{more}")

    term_count = len(terms)
    bp = len(broken_parents)
    print(f"verify: terms={term_count}, unique_ids={len(known)}, broken_parent_refs={bp}")
    if errors:
