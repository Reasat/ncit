...
build: $(OUTPUT_OWL) | dependencies
	...
	cp -f $(OUTPUT_OWL) $(OUTPUT_OWL_LINKML)
	@echo "Build complete: $(YAML_OUT), $(OUTPUT_OWL) (ROBOT), $(OUTPUT_OWL_LINKML) (release copy of ROBOT component)"

verify:
	python $(SCRIPTS_DIR)/verify.py --yaml $(YAML_OUT)

PREFIXES_METRICS=--prefix 'NCIT: http://ncicb.nci.nih.gov/xml/owl/EVS/Thesaurus.owl\#' \
